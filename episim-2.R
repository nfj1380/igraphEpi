...
episim <- function(G, nticks=10, beta=0.1, gamma=0.2, initState=NULL, propInfected=0.5) {
  ...
  inf_per_time<-data.frame(matrix(0,nticks+1,n))
  # Set initial state  
  ...
  t <- data.frame(matrix(0,nticks,n))#progression of infection/transition of state of each node for each time step
...
#  print(nextStat)
  for (tick in 1:nticks) {
    inf_per_time[time, ]=current_stat
    ...
    for (i in which(stat==1)) {#for all infected nodes in initial state
      if (runif(1)<=gamma) { #compares a randomly generated uniform number to recovery rate
        nextStat[i] <- 2;break #assigns node as recovered if condition above is met and break out of loop else
      }
      else{
