#'getGraphFeatures:  Wrapper to network features from single or mutliple network objects
#.
...
getGraphFeatures <- function(network_name, multinetwork = TRUE){
  
  if (multinetwork == TRUE) {
    
   networks <- list.files(paste(network_name))
...
#return(Global_summary)
  ...
  #} if(multinetwork == FALSE) {
    
  } else {
  
  dat <- read.table(network_name) #haven't dealt with the temporal nature of this yet. Third column appears to be a date...
    g <- graph_from_data_frame(dat)%>% as.undirected( "collapse")
    
    #g <- read_graph(networks[4], format = c("edgelist")) %>% as.undirected( "collapse") #seem only to work on smaller graphs?
    #one edge for each pair of connect vertices
    g_names <- gsub(".edges","",data)
    ...
    Global_summary <- as.data.frame(c(g_names, FiedlerValue, Adj_val, Rnot,network_size, most_infected_node, mod, deg, cent, trans, di)) 
    colnames(Global_summary) <- c('Network','Fiedler','Adj_val','Rnot','network_size','most_infected_node', 'Modularity', 'Mean_degree', 'Centrality', 'Transivity', 'Diameter')
