...
epi_summary<-function(f){    
   state_counts=data.frame(matrix(NA,nrow=(nticks+1),ncol=3))
   colnames(state_counts)=c("S","I","R")
   
    
   for (tick in 1:(nticks+1)){
   ...
   state_counts <- as.data.frame(t(state_counts))
   rownames(state_counts)=0:nticks
   state_counts
}  
...
for (i in 1:N){
#sim_all_graphs[[i]] = lapply(G_2,episim)
summary_all_graphs[[i]] = lapply(G_2,epi_summary)
}
#sim_all_graphs
summary_all_graphs
